    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/assistant/describe-image', {
                ...
                body: JSON.stringify({ image: base64Image, storeType: type, fileName: image.name }),
            });
            ...
            const searchTerms = query.toLowerCase().split(' ').filter(t => t.trim().length > 0);
            
            // Check if user is actively searching for an accessory
            const accessoryKeywords = ['case', 'charger', 'cover', 'cable', 'protector', 'adapter', 'mount', 'stand', 'strap'];
            const isAccessoryQuery = searchTerms.some(t => accessoryKeywords.includes(t));
            ...
            const matchedProducts = candidates.filter(product => {
                ...
                // Strictly drop accessories if the user didn't explicitly query for one
                if (!isFashion && !isAccessoryQuery) {
                    const isAccessoryProduct = accessoryKeywords.some(k => pNameLower.includes(k) || pCategoryLower.includes(k));
                    if (isAccessoryProduct) return false;
                }

                // Strict Matching Rules:
                
                // 1. Must match at least something in the name if query is multi-word
                // (prevents brand-only queries from matching all brand products incorrectly)
                if (searchTerms.length > 1 && nameMatches === 0) return false;

                // 2. Require stronger matches for multi-word queries to be very strict
                if (searchTerms.length >= 3 && totalMatches < 2) return false;
                if (searchTerms.length === 2 && totalMatches < 2) return false;

                if (expectedCategory) {
                    const categoryMatch = pCategory === expectedCategory;
                    // If in wrong category, require strict evidence
                    if (!categoryMatch && nameMatches < 2) return false;
                }
