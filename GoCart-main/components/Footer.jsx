...
const Footer = () => {
    ...
    useEffect(() => {
        ...
        const loadConfig = async () => {
            try {
                ...
                if (isMounted && data?.config && typeof data.config === "object") {
                    const raw = data.config
                    // Normalize in case old flat shape is returned
                    let next = DEFAULT_FOOTER_CONFIG
                    if (raw.electronics || raw.fashion) {
                        ...
                        const fashionBrand =
                            raw.brandDescriptionFashion ||
                            DEFAULT_FOOTER_CONFIG.fashion.brandDescription
                        next = {
                            electronics: {
                                ...DEFAULT_FOOTER_CONFIG.electronics,
                                ...base,
    ...
    const variantKey = isFashion ? "fashion" : "electronics"
    const variant = config[variantKey] || DEFAULT_FOOTER_CONFIG[variantKey]

    const linkSections = [
        {
            title: variant.productsSectionTitle,
            links: variant.productsLinks.map((item) => ({
                ...item,
                icon: null,
            })),
        },
        {
            title: variant.websiteSectionTitle,
            links: variant.websiteLinks.map((item) => ({
                ...item,
                icon: null,
            })),
        },
        {
            title: variant.contactSectionTitle,
            links: [
                { ...variant.contactLinks.phone, icon: PhoneIcon },
                { ...variant.contactLinks.email, icon: MailIcon },
                { ...variant.contactLinks.address, icon: MapPinIcon },
            ],
        },
    ]

    const socialIcons = [
        { icon: FacebookIcon, link: variant.socialLinks.facebook },
        { icon: InstagramIcon, link: variant.socialLinks.instagram },
        { icon: TwitterIcon, link: variant.socialLinks.twitter },
        { icon: LinkedinIcon, link: variant.socialLinks.linkedin },
    ]

    const trustBadges = [
        { label: variant.trustBadges[0]?.label, desc: variant.trustBadges[0]?.desc, Icon: Shield },
        {
            label: variant.trustBadges[1]?.label,
            desc: variant.trustBadges[1]?.desc?.replace("₹99+", `${currency}99+`),
            Icon: Truck,
        },
        { label: variant.trustBadges[2]?.label, desc: variant.trustBadges[2]?.desc, Icon: RefreshCw },
    ]

    const brandDescription = variant.brandDescription
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        {trustBadges.map((badge, i) => {
                            ...
                            if (!badge.label && !badge.desc) return null
                            return (
                                <div
                                    key={i}
                                    className={`flex items-center gap-4 p-4 rounded-xl ${
                                        isFashion
                                            ? "bg-white border border-[#d4c4a8]/30"
                                    ...
                                    <div>
                                        <p
                                            className={`font-semibold ${
                                                isFashion ? "text-[#2d1810]" : "text-zinc-200"
                                            }`}
                                        >
                                            {badge.label}
                                        </p>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6">
                <div
                    className={`flex flex-col md:flex-row items-start justify-between gap-10 py-12 ${
                        isFashion ? "text-[#8B7355]" : "text-zinc-400"
                    }`}
                >
                    <div className="max-w-md">
                        ...
                        >
                            <span className={isFashion ? "text-[#8B6914]" : "text-teal-400"}>go</span>
                            cart
                            <span className={isFashion ? "text-[#8B6914]" : "text-teal-400"}>.</span>
                        </Link>
                        <p
                                ...
                                item.link ? (
                                    <Link
                                        ...
                                        className={`flex items-center justify-center w-10 h-10 rounded-xl border transition ${
                                            isFashion
                                                ? "bg-white border-[#d4c4a8]/40 text-[#8B7355] hover:text-[#8B6914] hover:border-[#8B6914]/30"
                                                : "bg-zinc-800/80 border-zinc-700/80 text-zinc-400 hover:text-teal-400 hover:border-teal-500/40"
                                        }`}
                                    >
                                        <item.icon />
                                    </Link>
                ...
                <p
                    className={`py-6 text-sm ${
                        isFashion
                            ? "text-[#8B7355]/60 border-t border-[#d4c4a8]/30"
                            : "text-zinc-500 border-t border-zinc-800/80"
                    }`}
                >
                    {variant.copyrightText}
                </p>
            </div>
        </footer>
    )
