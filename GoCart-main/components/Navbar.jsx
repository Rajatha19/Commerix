...
const Navbar = () => {
    ...
    const storeItem = isFashion
        ? { label: isFashionSeller ? 'My Fashion Store' : 'Become a Vendor', path: isFashionSeller && fashionStoreUsername ? `/fashion/shop/${fashionStoreUsername}` : '/fashion/create-store', show: true }
        : { label: isSeller ? 'My Store' : 'Become a Vendor?', path: isSeller && electronicsStoreUsername ? `/shop/${electronicsStoreUsername}` : '/create-store', show: true };

    const menuItems = [
        { label: 'Stores', path: isFashion ? '/fashion/stores' : '/stores', show: true },
        storeItem,
        ...(isFashion ? [{ label: 'My Adjustments', path: '/fashion/adjustments', show: true }] : []),
        { label: isPro ? 'Pro Membership' : 'Become a Pro Member?', path: '/pro', show: true, isProLink: true },
                    ...
                    <div className={`relative flex items-center justify-between gap-4 transition-all duration-500 ${shouldExpand ? 'py-4 sm:py-5' : 'py-3 sm:py-3.5'
                        ...
                        <div className="flex items-center gap-2.5 order-1 min-w-0 flex-shrink-0 z-10">
                            <Link href={isFashion ? '/fashion' : '/'} className="flex items-center gap-2 flex-shrink-0 group relative">
                                ...
                                <span className={`font-bold leading-tight tracking-tight transition-all duration-500 ${shouldExpand ? 'text-2xl sm:text-3xl' : 'text-xl sm:text-2xl'
                                    ...
                                    <span className={isFashion ? 'text-[#8B6914]' : 'text-cyan-400'}>go</span>cart<span className={isFashion ? 'text-[#8B6914]' : 'text-cyan-400'}>.</span>
                                    {isPro && (
                                        <button
                                            type="button"
                                            onClick={(e) => {
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0 order-3 z-10 min-w-0">
                            ...
                            >
                                ...
                                <span className="hidden lg:inline text-sm font-semibold">{isFashion ? 'Electronics' : 'Fashion'}</span>
                            </Link>
