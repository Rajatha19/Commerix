...
export default function CartContent() {
    ...
    const getProductType = (product) => {
        ...
        const explicitType = product.productType || product.store?.storeType || product.type;
        if (!explicitType) return 'electronics';
        return explicitType === 'fashion' ? 'fashion' : 'electronics';
    };

    const getRatingStats = (ratingData) => {
        if (!ratingData) return { avg: 0, count: 0 };
        if (Array.isArray(ratingData)) {
            if (ratingData.length === 0) return { avg: 0, count: 0 };
            const sum = ratingData.reduce((total, r) => total + (r?.rating ?? 0), 0);
            return { avg: sum / ratingData.length, count: ratingData.length };
        }
        const numeric = Number(ratingData) || 0;
        return { avg: numeric, count: 0 };
    };
    ...
    useEffect(() => {
        if (!isPro || cartArray.length === 0) return;

        // Cross-vendor suggestions: only for electronics items (regardless of section) and for Pro users
        const itemsToFetch = cartArray.filter(
            (item) => item.type === 'electronics' && crossVendorSuggestions[item.id] === undefined
        );
        ...
        return () => { cancelled = true; };
    }, [isPro, getToken, Object.keys(cartItems).sort().join(','), cartArray.length, userLocation?.lat, userLocation?.lng, crossVendorSuggestions, dispatch]);
                        ...
                        <div className="flex items-start justify-between gap-8 max-lg:flex-col">
                            ...
                            <div className="w-full max-w-4xl space-y-6">
                                {cartArray.map((item, index) => {
                                    const showCrossVendor = isPro && item.type === 'electronics';
                                    const suggestion = showCrossVendor ? crossVendorSuggestions[item.id] : undefined;
                                    let p = suggestion?.suggestedProduct;
                                    ...
                                    const { avg: suggestedAvgRating, count: suggestedReviewCount } = getRatingStats(p?.rating);
                                    const { avg: cartAvgRating, count: cartReviewCount } = getRatingStats(item.rating);
                                    const isBetterPrice = p && p.price < item.price;
                                    const isHigherPrice = p && p.price > item.price;
                                    const hasBetterRating = p && suggestedAvgRating > cartAvgRating;
                                    const hasMoreReviews = p && suggestedReviewCount > cartReviewCount;
                                    if (p && isHigherPrice && !(hasBetterRating && hasMoreReviews)) {
                                        p = null;
                                    }
                                    const isBetterRating = p && hasBetterRating;
