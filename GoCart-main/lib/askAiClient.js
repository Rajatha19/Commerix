...
function getGroqClients() {
    const keys = [
        ...
        process.env.ASK_AI_GROQ_API_KEY_BACKUP_2?.trim(),
        process.env.ASK_AI_GROQ_API_KEY_BACKUP_3?.trim(),
        process.env.ASK_AI_GROQ_API_KEY_BACKUP_4?.trim(),
        process.env.ASK_AI_GROQ_API_KEY_BACKUP_5?.trim(),
        process.env.ASK_AI_GROQ_API_KEY_BACKUP_6?.trim(),
        process.env.ASK_AI_GROQ_API_KEY_BACKUP_7?.trim(),
        process.env.ASK_AI_GROQ_API_KEY_BACKUP_8?.trim(),
    ].filter(Boolean);
...
/**
 * Ask AI vision chat completions: forces Gemini (image support).
 * Use this when messages include image_url parts; Groq currently expects string-only content.
 */
export async function askAiVisionChatCompletionsCreate(options) {
    const gemini = getGeminiClient();
    const geminiModel = process.env.ASK_AI_GEMINI_MODEL?.trim() || "gemini-2.0-flash";
    if (!gemini) {
        throw new Error("Ask AI vision: set ASK_AI_GEMINI_API_KEY in .env");
    }
    return gemini.chat.completions.create({
        ...options,
        model: options.model || geminiModel,
